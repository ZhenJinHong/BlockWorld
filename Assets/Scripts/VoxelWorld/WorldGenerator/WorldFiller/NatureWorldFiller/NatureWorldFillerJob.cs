...
{
    [BurstCompile]
    public struct NatureWorldFillerJob : IJobParallelForBatch
    {
        public BlobAssetReference<NatureWorldFilling> WorldFilling;
        ...
        private Voxel GetVoxelID(int3 voxelIndexInWorld, byte height, float rangeValue, ref NatureWorldFilling filling)
        {
            ...
            if (y > height)
            {
                if (y == (height + 1))
                {
                    if (y > filling.WaterHeight)
                    {
                        ...
                        {
                            voxel = WorldMaths.GetVoxelByPercent(ref filling.Flowers, voxelIndexInWorld.xz, WorldMaths.RemapToZeroOne(flower));
                        }
                        else
                        {
                            voxel = WorldMaths.GetVoxelByPercent(ref filling.Grasss, voxelIndexInWorld.xz, WorldMaths.RemapToZeroOne(rangeValue));
                        }
                    }
                }
            }
            ...
            else
            {
                if (y < filling.WaterHeight)
                {
                    voxel = WorldMaths.GetVoxelByPercent(ref filling.UnderWaterBlocks, voxelIndexInWorld.xz, WorldMaths.RemapToZeroOne(rangeValue));
                }
                else
                    voxel = WorldMaths.GetVoxelByPercent(ref filling.SurfaceBlocks, voxelIndexInWorld.xz, WorldMaths.RemapToZeroOne(rangeValue));
            }
            if (y < filling.WaterHeight)
            {
